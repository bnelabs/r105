...
- Tabs are session-backed (Warp-style): Ctrl+Shift+T opens one on a fresh session, Ctrl+Shift+W closes it (or the focused pane in a split), Ctrl+Tab cycles, Alt+1..9 selects, and clicking the bar works too. Switching autosaves the live session first; the bar persists across restarts in `tabs.json`.
